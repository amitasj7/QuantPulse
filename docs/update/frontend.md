...
## update - 
🎯 The Goal: "Automated Commodity Synchronization & Live Streaming"The objective is to create a system where an Admin can add a commodity (like Gold/Silver) via the UI, and the system automatically fetches the correct Angel One Token, updates the Worker’s WebSocket subscription in real-time, and begins batch-flushing price ticks into TimescaleDB. 🗺️ Step-by-Step Execution PlanStep 1: Initialize the Broker IntegrationWe must first enable the backend to talk to Angel One using the credentials already in your .env. Install SDK: In your NestJS backend, install the official library: npm install smartapi-javascript.Auth Service: Create an AngelAuthService that uses your ANGEL_TOTP_SECRET to generate a session token every morning (since these tokens expire daily). Step 2: Build the "Master Token" EngineAngel One provides a massive list of symbols. We need to filter this so your Admin doesn't have to manually type in Token IDs.Fetch Script: Write a function that downloads the Angel One Master JSON.Filter Logic: Filter the JSON for exchange: "MCX".Local Cache: Save this filtered list (Token, Symbol, Expiry) into a local database table or Redis so searching is instant for the Admin.Step 3: Admin UI Interaction (Next.js)Search Input: Create a searchable dropdown in your Admin Panel that fetches from your "Filtered MCX List."Permission Check: Ensure only users with the ADMIN role can click the "Add to Pulse" button.Database Update: When "Gold" is added, the backend marks that symbol as isActive: true in your database.Step 4: Worker Hot-Reload (Redis Pub/Sub)This is where top-tier engineering comes in. You shouldn't restart the server to track a new stock.The Signal: When the Admin adds a stock, the Backend sends a message to Redis: {"action": "SUBSCRIBE", "token": "12345"}.The Listener: Your Worker (which is already running) listens to this Redis channel.Dynamic Update: The Worker receives the message and calls smartApiWebsocket.subscribe([new_token]) without dropping the existing connection.Step 5: Data Persistence (Batch Flushing)Buffer: The Worker collects price ticks for the new Gold/Silver token in an array.The 15s Flush: Every 15 seconds, the Worker executes a SQL INSERT to TimescaleDB, saving the high-frequency data for your charts.